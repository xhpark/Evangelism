...
자동 검증은 Flutter 테스트 61개, Apps Script 백엔드 통합 테스트 12개 그룹, 라인 커버리지 45% 하한, debug/release APK 빌드와 APK 서명 확인까지 포함합니다. 앱 버전은 `1.0.2+3`입니다. 2026-09-02 Galaxy S24 Ultra에서 새 서버 health 표식, 실제 일회용 코드 승인·소진, 동일 코드 재사용 거부, 포그라운드 복귀 후 토큰 승인과 최근 접속 갱신까지 확인했습니다. 강제 종료 후 재진입, 원격 차단, TTS/STT 항목은 통합 테스트 계획서의 남은 경계를 따릅니다.
