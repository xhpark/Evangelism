...
void main() {
  group('TTSService Syllable & Safe Overlap Tests (TS-TTS-001 ~ 004)', () {
    ...
    test('TS-TTS-001: 발화 시작 초기(0.5초 이내)에는 문장 전체 반환 (건너뜀 방지)', () {
      final remaining = TTSService.calculateRemainingText(sampleSentence, 0.4, 1.0);
      expect(remaining, equals(sampleSentence));
    });

    test('TS-TTS-002: 문장 중간(1.8초, 1.0배속) 발화 시 안전 중첩 어절 포함 검증', () {
      final remaining = TTSService.calculateRemainingText(sampleSentence, 1.8, 1.0);
    ...
    test('TS-TTS-003: 긴 문장에서 3.0초 경과 시 단어 건너뜀 없이 안전한 어절 유지', () {
      const longSentence = "선생님 만약 가장 절친한 친구가 선생님을 위해 값비싼 시계를 선물로 주었다고 생각해 보십시오";
      final remaining = TTSService.calculateRemainingText(longSentence, 2.5, 1.0);
    ...
    test('TS-TTS-004: 2.0배속 고속 발화 시에도 안전하게 이전 어절 포함', () {
      final remaining2x = TTSService.calculateRemainingText(sampleSentence, 1.2, 2.0);
