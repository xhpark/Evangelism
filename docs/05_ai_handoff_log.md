...
## 18. 2026-09-05 작업 기록 — 실전 STT 음성 암송 시험 채점 및 대조 알고리즘 전면 개편 & 성경 구절 암송 시험 제외

**작업 주체:** Antigravity / **의뢰인:** 박상환

### 18.1 요구사항 및 배경
1. **실전 STT 음성 암송 시험 채점 정확도 전수 검증 및 알고리즘 개편**:
   - STT로 변환된 텍스트와 원본 대본을 비교하여 점수를 매기고 단어별 대조 색상을 표시하는 부분이 신뢰하기 어렵다는 사용자의 우려 제기.
   - 기존 점수 산출 및 Diff 알고리즘을 철저히 감사한 결과, 다음 4가지 중대한 구조적 결함 확인:
     - **3단어 탐욕적 전방 탐색(Greedy 3-word lookahead)의 파탄**: 중간에 4단어 이상 건너뛰거나 문장을 빠뜨리면 인덱스 동기화가 완전히 깨져 뒤따르는 정상 발화 문장 전체가 빨간색(누락)과 주황색(추가)으로 오분류됨.
     - **공백(띄어쓰기) 불일치 감점**: STT 엔진이 "값 없이", "선물 입니다", "우주 만물"과 같이 띄어쓰기를 다르게 출력할 때 음절 편집거리와 키워드 판정에서 억울하게 대량 감점 발생.
     - **교재 원문에 없는 키워드 배치**: `just_ee_data.json`의 일부 스텝(예: `christ_2`에 대본에 없는 "죄의 책", `follow_1`에 "소책자" 등)에 원문에 없는 단어가 키워드로 등록되어 100% 완벽히 암송해도 만점이 불가능했던 문제.
     - **한국어 수사 표기 차이**: "2가지" ↔ "두 가지", "1년" ↔ "일 년", "하루 3번" ↔ "하루 세 번" 등 STT 출력과 대본 원문 간의 수사 표기 차이.
2. **실전시험에서 성경 구절 암송 제외**:
   - 실제 전도 현장에서는 복음 제시자가 성경 구절을 외워서 암송하기보다는 대상자에게 성경 책자를 펼쳐 직접 눈으로 보게 하거나 읽어 주므로, 실전 음성 시험에서는 순수 성경 구절 암송을 제외하기로 결정.
   - 3번 탭(성경덱)에서만 8대 핵심 구절 암송을 전담하도록 분리.
   - 실전시험(4번 탭)의 모든 모드(전환 연계, 서론/결신, 전체 완주 34문장, 무작위 모의고사)에서 순수 성경 구절 6개 스텝(`intro_4`, `grace_2`, `human_2`, `god_2`, `christ_3`, `faith_2`) 제외.
   - 단, 결신의 5번째 단계인 `commit_5`(구원의 확신 문답)는 단순 성경 암송이 아니라 "이 말씀에 근거하여 영생을 가졌는지" 확인하는 문답 대화이므로 실전시험에 유지.

### 18.2 주요 개선 사항
1. **LCS (최장 공통 부분 수열) DP 역추적 기반 Diff 알고리즘 전면 교체 (`ScoringEngine._generateDiffTokens`)**:
   - 2차원 DP 테이블과 역추적(Backtracking)을 적용한 LCS 알고리즘을 도입. 중간 문장이나 수십 단어를 통째로 건너뛰어도 뒤따르는 단어들을 정확히 `matched`로 동기화하여 공정하고 정확한 채점 및 시각적 색상 피드백 제공.
2. **공백 무시 음절 일치도 (Space-Resilient CER) 및 키워드 매칭 (`ScoringEngine.calculateScore`)**:
   - 띄어쓰기를 완전히 제거한 텍스트 기준의 음절 레벤슈타인 거리(`charAccNoSpace`)를 기존 거리와 가중 결합하여 STT 임의 띄어쓰기로 인한 감점 차단.
   - 키워드 검출 시에도 공백을 제거한 형태를 동시 검사하고, 대본 원문에 실제 존재하는 키워드만 유효 키워드로 필터링.
3. **어간 판정 (`isSameWordStem`) 및 조사/어미 규칙 확장**:
   - '입니다', '습니다', '하셨습니까', '하셨어요', '이에요', '예요', '에게' 등 구두 대화체 어미 및 조사를 지원하여 문맥상 동일한 단어를 정확히 정답 인정.
4. **한국어 수사 및 서수사 표준 정규화 (`KoreanTextNormalizer.normalizeNumerals`)**:
   - STT가 "2가지", "1년", "첫번째", "하루 3번", "9만 번" 등으로 인식해도 교재 대본 표준 표기인 "두 가지", "일 년", "첫째", "하루 세 번", "9만 번"으로 상호 정규화.
5. **대본 키워드 정비 (`data/just_ee_data.json`)**:
   - `intro_2`, `intro_4`, `human_3`, `human_4`, `god_1`, `christ_2`, `christ_3`, `faith_3`, `commit_1`, `commit_2`, `commit_5`, `follow_1`, `follow_2`, `follow_3`, `follow_4`의 키워드를 실제 대본 어휘와 100% 일치하도록 전수 정비.
6. **실전시험 엔진 및 4대 영역 성적표 재편 (`RandomExamEngine`)**:
   - `ExamMode.scriptureChain` 모드 삭제 (총 6대 모드로 개편).
   - `ExamMode.fullSequential`을 순수 성경 구절 6개를 제외한 34문장으로 최적화.
   - 5대 영역 평가 성적표에서 성경구절(25점)을 제외하고 실전 4대 핵심 영역(교리 35점, 대지 전환문장 25점, 예화 25점, 결신/양육 15점 = 총 100점)으로 재할당.
7. **단위 및 회귀 테스트 추가/갱신**:
   - `test/scoring_rules_test.dart`: `TS-SCORE-008`(띄어쓰기 복원력), `TS-SCORE-009`(LCS 문장 건너뛰기 Diff), `TS-SCORE-010`(수사/서수사 정규화) 신규 추가.
   - `test/random_exam_engine_test.dart`: `TS-RAND-003`(순수 성경구절 실전시험 제외 및 34문장 검증), `TS-RAND-005`(4대 영역 성적표 산출 검증) 갱신.
