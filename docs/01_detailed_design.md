...
**문서 버전:** v2.3 (2026-08-29 전면 코드 점검 및 보안·안정성 보강 반영)  
**작성일:** 2026-08-29  
**최종 검증:** 2026-08-29 — 실제 소스·데이터와 1:1 대조 완료 (`flutter analyze` 무결점, `flutter test` 42개 통과)  
...
* **2026-08-29 보안 개정 (전면 점검 반영)**:
  - **인증키 우회 분기 제거**: `"JUST"`로 시작하고 8자 이상이면 무조건 통과시키던 해시 분기를 삭제했습니다. 이제 내장 마스터 키 목록과 정확히 일치할 때만 활성화됩니다. (회귀 테스트 `TS-SEC-001`이 우회 시도 5종을 차단하는지 매 실행 검증)
  - **요청 서명(sig) 도입**: 앱은 `SHA-256("<기기UUID>|<공유 시크릿>")` 서명을 함께 보내고, Apps Script가 이를 검증합니다. 서명이 없거나 틀리면 시트 등록·조회가 거부됩니다.
  - **서버측 인증키 재검증**: 백엔드의 `MASTER_PINS`가 선언만 되어 있고 실제로 쓰이지 않던 문제를 고쳐, 유효하지 않은 인증키의 등록 요청은 `DENIED`로 반려합니다.
  - **개발자 전용 설정 보호**: 설정 탭의 구글 시트 연동 주소 변경은 마스터 인증키 재입력을 통과해야 열리며, `https://...google.com` 형식만 저장됩니다. (이전에는 훈련생이 주소를 비워 킬 스위치를 영구 무력화할 수 있었습니다)
  - **백업 복제 차단**: `android:allowBackup="false"` 및 `data_extraction_rules.xml`로 클라우드 백업·기기 간 전송에서 라이선스 데이터를 제외했습니다.
  - **미등록 단말 자가 재등록**: 원격 조회 결과가 `UNREGISTERED`인데 로컬은 활성 상태이면 앱이 스스로 재등록해, 앱 데이터를 지우고 재설치한 단말도 관리 시트에 다시 나타나 차단 대상이 됩니다.
  - **상시 재확인**: 앱 시작 시 1회만 조회하던 킬 스위치를 포그라운드 복귀 시마다 재확인하고, 사용 중 차단되면 즉시 차단 화면으로 전환합니다.
