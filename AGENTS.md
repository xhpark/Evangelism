...
3. **되살리면 안 되는 것**: 즉석 양육 마스터 화면 계열(`FollowUpMasterScreen`/`FollowUpProvider`/`FollowUpEngine`), 설정 화면의 "교재 기본 대본 전체 복원" 버튼. 둘 다 의도적으로 제거됨.
4. **비밀 값 금지**: 마스터 인증키(`LicenseService._masterPins`)와 웹훅 URL의 실제 값을 문서·커밋 메시지·이슈에 기재하지 않는다.
