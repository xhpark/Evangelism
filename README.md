...
[![Tests](https://img.shields.io/badge/Tests-61%20Passed-brightgreen)](docs/02_module_test_specs.md)
...
1. **🎧 1. 학습/청취 (Study & Listen)**
   * 0.8x ~ 2.0x (0.8x, 1.0x, 1.2x, 1.5x, 2.0x) 속도 가변 TTS 고음질 음성 재생.
...
4. **🎙️ 4. 실전시험 (Real Voice Exam)**
   * 7대 실전 시험 모드 (전환 ➔ 다음 단락 연계, 예화 집중 완주, 성경 구절 암송, 서론/결신 문답, 즉석 양육 항목별, 무작위 모의고사, 전체 전문 100% 완주 34문장).
   * 연계/완주 모드에서는 실전 특성에 맞춰 순수 성경 구절 암송을 제외하되, **성경 구절 암송 모드에서는 핵심 성경 구절의 정확한 암송을 집중 테스트**.
...
* **[🧪 단위 및 모듈 테스트 명세서](docs/02_module_test_specs.md)**: 61개 단위/위젯 테스트 통과 내역
...
자동 검증은 Flutter 테스트 61개, Apps Script 백엔드 통합 테스트 12개 그룹, 라인 커버리지 45% 하한, debug/release APK 빌드와 APK 서명 확인까지 포함합니다. 앱 버전은 `1.0.3+4`입니다. 2026-09-02 Galaxy S24 Ultra에서 새 서버 health 표식, 실제 일회용 코드 승인·소진, 동일 코드 재사용 거부, 포그라운드 복귀 후 토큰 승인과 최근 접속 갱신까지 확인했습니다. 강제 종료 후 재진입, 원격 차단, TTS/STT 항목은 통합 테스트 계획서의 남은 경계를 따릅니다.
